...
interface IIndexToken {
    function indexTokens(uint256 index)
        external
        view
        returns (address token, uint256 weighting, uint256 basePriceUSDX96, uint256 c1, uint256 q1);
}

interface ICamelotLPToken {
    function addLiquidityV2(uint256 _idxLPTokens, uint256 _pairedLPTokens, uint256 _slippage, uint256 _deadline)
        external;
}
