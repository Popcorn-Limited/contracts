...
contract PendleAdapter is AdapterBase {
    ...
    function initialize(
        ...
        (pendleMarket) = abi.decode(pendleInitData, (address));
    ...
    function _totalAssets() internal view override returns (uint256 t) {
        ...
        // apply slippage
        t = totAssets - totAssets.mulDiv(0.01e16, 1e18, Math.Rounding.Floor);
    }

    function refreshRate() public {
        // for some reason the call reverts if called multiple times within the same tx
        try pendleOracle.getLpToAssetRate(address(pendleMarket), 900) returns (uint256 r) {
    ...
    function _protocolDeposit(
        ...
        (uint256 n, , ) = pendleRouter.addLiquiditySingleToken(
            address(this), 
            pendleMarket, 
            0, 
            approxParams, 
    ...
    function _protocolWithdraw(
        ...
        // TODO FIX else case to an amount that doesn't consider slippage
        uint256 lpAmount = amount == totalAssets() ? IERC20(pendleMarket).balanceOf(address(this)) : amountToLp(amount);
