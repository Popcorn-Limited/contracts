...
contract VaultWithRewards is Vault {
    ...
    function _accrueUser(address _user) internal {
        ...
        for (uint i; i < len;) {
            address[] memory _tokens = IStrategy(strategies[i].addr).getRewardTokens(); 
            for (uint j; j < _tokens.length;) {
                ...
                if (oldIndex == 0) {
                    oldIndex = 10 ** IERC20Metadata(address(_rewardToken)).decimals();
                }
    ...
    function mint(uint shares, address receiver) public override returns (uint assets) {
