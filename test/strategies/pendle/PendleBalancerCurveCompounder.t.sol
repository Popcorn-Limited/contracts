...
contract PendleBalancerCurveCompounderTest is BaseStrategyTest {
    ...
    function setUp() public {
        _setUpBaseTest(0, "./test/strategies/pendle/PendleBalancerCurveCompounderTestConfig.json");
    }
    ...
    function test_depositWithdraw() public {
        ...
        uint256 shares = IERC20(address(strategy)).balanceOf(address(bob)).mulDiv(2e17, 1e18, Math.Rounding.Ceil);

        strategy.redeem(shares, bob, bob);
        assertEq(IERC20(strategy.asset()).balanceOf(bob), totAssets.mulDiv(2e17, 1e18, Math.Rounding.Floor));
    ...
    function _setHarvestValues(string memory json_, string memory index_, address payable strategy) internal {
        ...
        address curveRouter_ = json_.readAddress(string.concat(".configs[", index_, "].specific.harvest.curveRouter"));
        ...
        PendleBalancerCurveCompounder(payable(strategy)).setHarvestValues(
            balancerVault_, tradePaths_, curveRouter_, swaps_
        );
    }
