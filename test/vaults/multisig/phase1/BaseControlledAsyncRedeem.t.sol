...
import {BaseControlledAsyncRedeem} from "src/vaults/multisig/phase1/BaseControlledAsyncRedeem.sol";
...
contract BaseControlledAsyncRedeemTest is Test {
    ...
    function setUp() public {
        ...
        vault = new MockControlledAsyncRedeem(owner, address(asset), "Vault Token", "vTEST");

        // Setup initial state
        asset.mint(alice, INITIAL_DEPOSIT);
        vm.startPrank(alice);
        asset.approve(address(vault), type.max);
    ...
    function testRequestRedeem() public {
        ...
        RequestBalance memory balance = vault.requestBalances(alice);
