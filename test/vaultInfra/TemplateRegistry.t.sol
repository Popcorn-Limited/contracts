...
contract TemplateRegistryTest is Test {
 ...
 function test__addTemplate() public {
   ...
   assertTrue(registry.templates(version, template));
 }

 function test__addTemplate_template_already_exists() public {
   address template = vm.addr(1238);
   addTemplate(template);

   vm.expectRevert(abi.encodeWithSelector(TemplateRegistry.TemplateExists.selector, version, template));
   registry.addTemplate(
     version,
     templateCategory,
     template
