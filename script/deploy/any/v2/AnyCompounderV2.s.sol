...
import {AnyCompounderV2, IERC20} from "src/strategies/any/v2/AnyCompounderV2.sol";
import {PendingTarget, CallStruct} from "src/strategies/any/v2/AnyConverterV2.sol";

contract Deploy is Script {
    using stdJson for string;

    function run() public returns (AnyCompounderV2 strategy) {
        string memory json = vm.readFile(
            string.concat(
                vm.projectRoot(),
                "/script/deploy/any/v2/AnyCompounderV2DeployConfig.json"
            )
        );
        ...
        targets = json.readAddressArray(".strategyInit.pendingAllowances.address");
