...
contract DeployStrategy is Script {
    ...
    function run() public returns (PendleBalancerCurveCompounder strategy) {
        string memory json = vm.readFile(
            string.concat(vm.projectRoot(), "/script/deploy/pendle/PendleBalancerCurveCompounderDeployConfig.json")
        );
    ...
    function _setHarvestValues(string memory json, address payable strategy) internal {
        ...
        // Set harvest values
        PendleBalancerCurveCompounder(strategy).setHarvestValues(balancerVault_, tradePaths_, curveRouter_, swaps_);
    }
